...
const PATH_AUTH = `${process.env.REACT_APP_API_AUTH_PATH}`;
//headerde conexion
  const config = {
    headers: {
...
//actualizar pagina
  const updatePage = async ()=>{
      try{
    const { data } = await axios.get( `${BASE_URL}${PATH_AUTH}`, config);
    setUserData(data);
      }
      catch{
console.log(error);
...
///Borrar usuario
  const deleteUser = async (userId) => {
    ...
    if (response) {    
         ...
         axios.delete(`${BASE_URL}${PATH_AUTH}/${userId}`,config);
...
const actualizarUser = async (userId) => {
  ...
  if (response) {
   ...
   var rolesAct=document.getElementById("roles2").value
        const data= await axios.put(`${BASE_URL}${PATH_AUTH}/${userId}`,
