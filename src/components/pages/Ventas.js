...
//actualizar pagina
  const updatePage = async ()=>{
    try{
    const { data } = await axios.get(`${BASE_URL}${PATH_VENTAS}`, config);
    setVentasData(data);
...
const listaProductos= async ()=>{
    try{
        const { data } = await axios.get(`${BASE_URL}${PATH_PRODUCTS}`, config);
        setProductData(data);
...
///Borrar Venta
  const deleteVenta = async (ventasId) => {
    ...
    if (response) {
        ...
        await axios.delete(`${BASE_URL}${PATH_VENTAS}/${ventasId}`,config);
...
const actualizarVenta = async (ventasId) => {
  ...
  if (response) {
   ...
   var carritoAct=document.getElementById("carrito2").value
     ...
     const data= await axios.put(`${BASE_URL}${PATH_VENTAS}/${ventasId}`,
